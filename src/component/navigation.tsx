    ...
    return (
        ...
        >
            <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center space-x-8">
                    {navItems.map((item) => (
                        <motion.a
                            key={item.name}
                            href={item.href}
                            className={`transition-colors font-[var(--font-open-sans)] ${pathname === item.href ? "text-primary font-semibold" : "text-muted-foreground hover:text-foreground"
                                }`}
                            whileHover={{ y: -2 }}
                        >
                            {item.name}
                ...
                {
                    session.data && <Link href={"/dashboard"}><span>View Dashboard</span></Link>
                }
